...
TEST_F(Factory_Test, can_add_workers)
{
  testee.add("charlie");
  ASSERT_EQ(std::set<std::string>{"charlie"}, testee.workers());

  testee.add("joe");
  ASSERT_EQ(std::set<std::string>({"charlie", "joe"}), testee.workers());
}
...
{
  int product42 = 42;
  testee.add("42", [&product42]{return &product42;});
  int product57 = 57;
  testee.add("57", [&product57]{return &product57;});

  ASSERT_EQ(&product42, testee.produce("42"));
  ASSERT_EQ(&product57, testee.produce("57"));
}
